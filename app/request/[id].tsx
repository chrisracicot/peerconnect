...
export default function RequestDetailScreen() {
  ...
  const loadRequest = async () => {
    try {
      ...
      if (data) {
        ...
        setEditedCourseId(data.course_id);
      }
    } catch (error) {
  ...
  const handleCancel = () => {
    if (request) {
      ...
      setEditedCourseId(request.course_id);
    }
    setIsEditing(false);
  };
        ...
        <View style={styles.content}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detail}>Course: {isEditing ? editedCourseId : request.course_id}</Text>
            </View>
            <View style={styles.detailRow}>
          ...
          <View style={styles.buttonsContainer}>
            {request.user_id !== user?.id ? (
              /* Foreign Request: Show Message Button only if pending */
              request.status === 'pending' ? (
                <TouchableOpacity
                  style={styles.messageButton}
                  onPress={() => router.push({
                    pathname: `/(tabs)/messages/${request.user_id}` as any,
                    params: {
                      initialMessage: `Hi, I'd like to help you with your request: ${request.title}`
                    }
                  })}
                >
                  <FontAwesome name="paper-plane" size={16} color="#fff" />
                  <Text style={styles.messageButtonText}>Message Student to Help</Text>
                </TouchableOpacity>
              ) : (
                <View style={styles.closedMessage}>
                  <Text style={styles.closedMessageText}>This request is no longer available.</Text>
                </View>
              )
            ) : (
              /* Own Request: Show Management Buttons */
              !isBrowseContext && (
                isEditing ? (
                  <>
                    <TouchableOpacity
