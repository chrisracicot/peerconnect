...
export default function BookingsScreen() {
    ...
    const updateBookingStatus = async (status: "pending" | "confirmed" | "completed" | "canceled") => {
        if (!selectedBooking) return;

        if (status === "confirmed") {
            // Hide status modal and show payment modal instead
        ...
        await finalizeBookingStatus(status);
    };

    const finalizeBookingStatus = async (status: "pending" | "confirmed" | "completed" | "canceled") => {
        if (!selectedBooking) return;
        ...
        >
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContainer}>
                        ...
                        <View style={styles.statusButtonsContainer}>
                            ...
                            >
                                <Text style={styles.statusButtonText}>Confirm (Pay)</Text>
                            </Pressable>
            ...
            {selectedBooking && userId && (
                <PaymentModal
                    ...
                    amount={25.0} // Fixed demo amount
                    senderId={userId}
                    receiverId={
                        selectedBooking.requester_id === userId
                            ? selectedBooking.provider_id
                            : selectedBooking.requester_id
                    }
                    referenceId={selectedBooking.id.toString()}
                    onClose={() => setShowPaymentModal(false)}
                    onSuccess={() => {
                        setShowPaymentModal(false);
                        finalizeBookingStatus("confirmed");
                    }}
                />
            )}
            ...
            {selectedBooking && userId && (
                <ReviewModal
                    ...
                    onSuccess={() => {
                        ...
                        finalizeBookingStatus("completed");
                    }}
                />
            )}
        </ScrollView>
